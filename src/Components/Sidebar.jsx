...
const Sidebar = ({ currentProductType }) => {
  ...
  const handleFilterSelect = (filterType, filterValue) => {
    ...
    // Apply filters but maintain the current product type route if we're on a product type page
    if (location.pathname.includes('/products/type/')) {
      // Extract the current product type from URL
      const pathParts = location.pathname.split('/');
      const typeIndex = pathParts.indexOf('type');
      
      if (typeIndex !== -1 && typeIndex + 1 < pathParts.length) {
        // Stay on the same product type page but with updated filters
        // No navigation needed, just apply filters
        applyFilters();
      }
    } else {
      // If we're not on a specific product type page, just apply filters
      applyFilters();
    }
  };
  ...
  const filtersData = {
    ...
    materials: "14K Gold,18K Gold,Acrylic,Alloy,Brass,Stainless Steel"
  };
